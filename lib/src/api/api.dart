...
class ApiProvider {
  ...
  final _apiWeatherUrl = 'http://samples.openweathermap.org/data/2.5/weather?';
  ...
  Future<LocationModel> fetchLocationData(String cityName) async {
    var encodedCityname = cityName.replaceAll(' ', '%20');
    var url = '$_apiMapsUrl$encodedCityname&key=$_apiKeyMaps';
    var response = await client.get(url);
